    ...
    : QMainWindow(parent)
{
    ...
    //    tbtn[i]->setFont(font);
        //tbtn[i]->setText("xiaf");
        tbtn[i]->setIcon(QIcon(":/ImageIcon/20.png"));
    ...
    QString strLabelUnselected[6]={":/ImageIcon/30.png", ":/ImageIcon/30.png", ":/ImageIcon/30.png",
                                   ":/ImageIcon/30.png", ":/ImageIcon/30.png", ":/ImageIcon/30.png"};

    for(int i=0;i<6;i++)
    {
        QMovie *movie=new QMovie("C:/Users/20200062/Documents/Qt_Programme/Qt_test/QtOceanChart/ImageIcon/66.gif");
    ...
    for(int i=0; i<6; i++)
    {
        ...
        series[i]->append(0,2);
        series[i]->append(QPointF(2,6));
        series[i]->append(3,8);
        series[i]->append(7,i*2);
        series[i]->append(11,3);

        *series[i] << QPointF(11,2) << QPointF(15,5) << QPointF(18,4) << QPointF(19,i);
        ...
        chart[i]->setTitle(QStringLiteral("Qt line chart example"));

        viewChart[i] = new QChartView(chart[i]);
        viewChart[i]->setParent(this);
        viewChart[i]->setRenderHint(QPainter::Antialiasing);
        viewChart[i]->resize(400,300);
        viewChart[i]->move(1190,400);
        viewChart[i]->hide();
...
void MainWindow::paintEvent(QPaintEvent*)
{
    ...
    QPixmap pix;
    pix.load(":/ImageIcon/155.png");//图片的位置
